...
function CreateTCP139FWRule{
    Try{
    Invoke-command -computer $global:globalhostname {netsh advfirewall firewall add rule dir=in action=allow protocol=TCP localport=139 name="CE_Allow_TCP_139_private" profile=private}
...
## Option 8  EnableAutoShareServer 
...
function Get-Variables{
    ...
    Write-Host "│ Version   : 0.1 (Very Alpha)                         │"
...
## It basically all starts with asking the user for input, all submenus follow, so effetvely this is the program "Run" function.  
## To add: If statement, if user specifices variables, do not request them and continue with execution of the program 
